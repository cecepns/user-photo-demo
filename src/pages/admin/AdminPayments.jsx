...
const AdminPayments = () => {
  ...
  const handleSubmit = async (methodData) => {
    try {
      const url = editingMethod 
        ? `https://api-inventory.isavralabel.com/chekusphoto/api/payment-methods/${editingMethod.id}`
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`https://api-inventory.isavralabel.com/chekusphoto/api/payment-methods/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('admin_token')}`
        }
